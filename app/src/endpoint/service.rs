...
pub async fn register_mail_confirm(
    ...
    let sr = utils::sendmail(&reg_mail_auth_req.mail, &result.to_string(), &endex.conf);
    if sr == 0 {
...
pub async fn register_oauth_github(
    ...
    let email = parse_oauth_profile(&oauth_result);
    persistence::insert_user_cond(
        &endex.db_pool, 
        persistence::UserCond {
...
fn parse_oauth_profile(oauth_result: &str) -> String {
    let parsed: Value = serde_json::from_str(oauth_result).unwrap(); 
    let obj: Map<String, Value> = parsed.as_object().unwrap().clone();
    println!("access obj {:?}", obj);
    let github_id: String = obj.clone().get("id").unwrap().to_string();
    github_id
}
...
mod tests {
    ...
    fn test_parse_oauth_profile() {
        let oauth_profile = "{\"login\":\"someone\",\"id\":12345,\"node_id\":\"some_id\",\
            \"avatar_url\":\"https://avatars.githubusercontent.com/u/12345?v=4\",\"email\":\"some@some.com\",\
            \"plan\":{\"name\":\"free\",\"space\":12345,\"collaborators\":0,\"private_repos\":10000}}";
        let github_id = parse_oauth_profile(oauth_profile);
